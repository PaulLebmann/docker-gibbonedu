...
mariadb-dump -u root --password=$BACKUP_DB_ROOT_PW $BACKUP_DB_NAME | gzip > "/backup/backup-db-"$BACKUP_DB_NAME"-"$BACKUP_TIME".sql.gz"
BACKUP_TIME_PRINT=$(date +"%Y-%m-%d %H:%M")
echo "$BACKUP_TIME_PRINT => Backed up database" 
